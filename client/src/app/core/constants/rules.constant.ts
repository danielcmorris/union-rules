...
The algorithm evaluates each job record in strict priority order. The first rule that matches determines the classification — lower-priority rules are not considered.

| Priority | Condition | Result |
|---|---|---|
| **1 (highest)** | Gap from prior timesheet was between 4 and 8 hours (PT Lock) | All hours on this timesheet = **PT**, regardless of any other rule |
| **2** | Job falls on Saturday, Sunday, or a union holiday | All hours on that job = **DT** |
| **3** | Pay type 2 or 7 — within the first 4 paid hours of the shift | **PT** |
| **4** | Pay type 1, 3, 4, 5, or 6 — effective pre-shift hours ≥ 6h | All hours on this timesheet = **PT** |
| **5** | Hours before 6:00 AM (pre-shift, after pre-shift threshold cleared) | **PT** |
| **6** | Hours within the Standard Time Window (up to 8 paid hours) | **ST** |
| **7 (lowest)** | Hours after the Standard Time Window closes | **PT** |

> **PT Lock (Priority 1) overrides Double Time (Priority 2).** If the gap from the prior timesheet was 4–8 hours and the current timesheet falls on a Saturday, the hours are still classified as **PT** (not DT) by the algorithm. This is an edge case that rarely occurs but is important to state explicitly.
...
> **Important — Meal Counter Continuity:** The meal counter carries forward across consecutive jobs **only when there is no time gap between them**.
> - If Job A ends at 3:00 PM and Job B starts at **3:00 PM** (no gap) → hours **pool**. The formula is applied to the combined paid hours of the run, and any meals already counted in Job A are subtracted to find the new meals earned by Job B.
> - If Job A ends at 3:00 PM and Job B starts at **5:00 PM** (gap exists) → the counter **resets** at Job B's start. The formula is applied only to Job B's own paid hours.
> - Any gap at all (even one minute) between a job's end time and the next job's start time resets the counter.
...
> - 11h → 1 missed meal
> - 15h → 1 missed meal (exactly at the 2nd threshold — boundary rule applies, must work *past* 15h to earn a 2nd)
> - 15.1h → 2 missed meals (0.1h past the 2nd threshold)
> - 19.5h → 2 missed meals (exactly at the 3rd threshold — boundary rule, must work past it)
> - 19.6h → 3 missed meals (0.1h past the 3rd threshold)
...
When timesheets are linked (gap < 4 hours), each TimeSheet ID still earns its own $50 subsistence payment independently — linking does not merge two timesheets into a single subsistence payment.
...
The extra hours are paid at the **same rate** as the actual hours worked. On a mixed-rate timesheet (e.g., some hours at ST and some at PT), the padded guarantee hours are added at the rate of the **last time segment** — whatever classification the employee was earning at the end of their final job.
...
| More than 8, up to 10 hours (four-tens schedule) | 10 hours |
| More than 8 hours (standard schedule), or more than 10 hours (four-tens) | Actual hours — no padding |

---

### 7. Minimum Call-Out

The contract provides a minimum call-out guarantee. In the current calculation algorithm, this minimum billing guarantee is enforced through the **Show-Up Pay** mechanism (Section 6 above), which applies to the **first timesheet of the calendar day only**.

> **Example:** An employee is called in for a scheduled shift at 6:00 AM. It rains at 6:15 AM and the foreman sends everyone home. Because this is the first timesheet of the day, show-up pay applies: 15 minutes worked rounds up to the 2-hour minimum billed.

> **Important:** The current algorithm does **not** apply a separate minimum call-out floor to timesheets that are not the first of the calendar day. If you see a subsequent timesheet with fewer than 4 billed hours, this is not an error — show-up pay only applies once per calendar day. Verify the underlying timesheet records if a dispute arises over minimum call-out on subsequent call-outs.
